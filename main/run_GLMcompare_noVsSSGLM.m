...
for sbj = 1:numel(sbjfolder) % loop across subjects
    ...
    len = size(AUX,1);
    spltIDX = {1:floor(len/5),floor(len/5)+1:floor(len*3/5), floor(3/5)+1:len};
...
if flag_save
    disp('saving data...')
    save([path.save '\FV_results_opt.mat'], 'FMdc', 'FMss', 'FMcca', 'FWss', 'FWcca', 'TTM', 'lstHrfAdd', 'lstLongAct', 'lstShortAct', 'FMclab');
end
